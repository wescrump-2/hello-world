...
export class Deck {
	...
	public cleanupOrphanCards() {
		...
		for (const card of this.carddeck) {
			if (typeof card.pile === 'object' && typeof card.pile.characterId === 'string') {
		...
		if (cleaned > 0) {
			console.log(`Returned ${cleaned} orphaned cards.`)
	...
	try {
		...
		if (err.message?.includes("metadata") && err.message?.includes("16")) {
			...
			try {
				...
				console.log("Corrupted metadata cleared");
				...
				console.log("Fresh deck saved after cleanup");
	...
	public async performSaveNow(): Promise < void> {
	const data = this.Meta;
	try {
		// Ensure scene is ready before saving metadata
		await Util.ensureSceneReady();
...
shuffle() {
	...
	if (pile && typeof pile === 'object' && typeof pile.characterId === 'string') {
		const hasPlayer = this.players.has(pile.characterId);
		const result = hasPlayer ? pile : 'discard';
		return result;
	}
...
moveToPool(to: PileId, from: PileId, numCards = 0, fromTop = true, dir = Facing.None) {
	...
	const source = this.carddeck
		.filter(c => {
			const matches = Deck.isSamePile(c.pile, from);
			return matches;
		})
